...
The Bulletproof contains the Pedersen commitment discussed above, as well as provides a range proof on the amount, $x$. Everyone knows the $E_q$ group generators, $G$ and $A$. Alice's complete transaction record contains additional information that we will discuss below.
...
When creating $UTXO_D$, Alice must choose random values $\alpha, k \in Z_r$ that will be used in cloaking the symmetric data encryption key, before storing it as $Key_{BLOB}$. The actual symmetric key for the encrypted data object will be $H_r(k \, G)$. Alice encrypts her data with AES using the computed symmetric key and places the encrypted object along with its size and time-to-live into $UTXO_D$.
