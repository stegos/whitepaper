...
\begin{flushleft}
	...
	{\sffamily\color{white}\fontsize{16}{20}\selectfont
	\@author\\
	\@date}
...
\author{Stegos AG}
\date{\today \\v1.0\\ \vspace{10pt}\colorlet{urllinkcolor}{white}\url{https://github.com/stegos/whitepaper}}
...
\todo[inline]{Describe why smart contracts are not sufficient}
...
Stegos is environmentally friendly and does not waste megawatts of electricity mining blocks. Instead, Stegos uses a bespoke egalitarian proof-of-stake (ePoS) consensus, building on advancements in distributed systems theory and cryptography. Each new Stegos block must be verified and confirmed by a group of validators, all of whom must post tokens as bonds. The value of the tokens staked in this way directly affects the probability that a validator will win a block and earn the associated transaction fees. In addition, a randomly awarded validator service award ensures that validators at every level of stake are incentivized to maintain the network. A portion of the reward from each block is added to the service award pool, which continues to grow until a winner is selected in an ongoing cryptographic lottery based on verifiable distributed randomness. 
...
\subsection{Egalitarian Proof-of-stake (ePoS) consensus}
Stegos is environmentally sustainable and does not require terawatts of electricity to be spent mining blocks. To achieve this, Stegos uses a unique consensus mechanism we call egalitarian proof-of-stake (ePoS), building on ideas from pBFT\cite{c9} and Collective Signing\cite{c10}\cite{c11}. Each new Stegos block must be verified and confirmed by a group of validators, all of which must put up a performance bond or stake. The size of the coins staked has a direct effect on the probability that a given validator will become the leader of a consensus round and thus earn block rewards and transaction fees.

ePoS is egalitarian because every validator which has been running for a certain amount of time has the chance to receive a $Validator service award$, regardless of the size of their stake\footnote{A nominal minimum threshold will apply to prevent users from gaming the system, but since every eligible node must provide validation services to qualify, it is impossible to spam the system, as one goal of the $Validator service award$ is to maximize the number of different nodes.}.
...
\subsection{Pruning for data compaction}
...
\subsection{Data messaging}
Unlike other blockchains, Stegos does not restrict users to sending just payments. Stegos supports FastData messaging, allowing users to exchange data messages with the same privacy guarantees afforded to payments. In fact, because data cannot be meaningfully ``double spent", sending data messages is actually much faster than sending payments because it requires fewer confirmations. Data is a first-class citizen on the Stegos blockchain, which is only logical as the average user is expected to send many more data transactions than standard payments.

\subsection{Privacy applications}
...
\subsubsection{Stegos mobile app}
...
\todo[inline]{Merge egaletarian PoS}
%%
The Stegos egalitarian proof-of-stake (ePoS)consensus protocol is based on pBFT\cite{c9} but adds strong consistency, which enables all validators to agree on the validity of blocks without wasting computational power resolving inconsistencies. 
%%
...
Stegos is a public so anyone can join the network, become a validator and earn rewards for maintaining the blockchain. A validator is any node that has staked tokens, i.e. posted a performance bond. Requiring a performance bond helps against Sybil attack by requiring a significant financial commitment from validator.
...
\todo[inline]{Merge leader election}
Stegos is a public ledger, so anyone can join the network, become a validator and earn rewards for maintaining the blockchain. Stegos uses egalitarian proof-of-stake (ePoS) consensus and all validators need to post a performance bond (stake). Requiring a performance bond protects against Sybil attacks by requiring a financial commitment from validators.%Originally this said "significant" commitment, but doesn't that run contrary to claims about ePoS being available to almost everyone?
...
%%
\todo[inline]{Merge description of CoSi}
...
%%
\section{Networking}
\todo[inline]{Add Kademlia and RaptorQ}
...
\section{Randomness}
\todo[inline]{Describe our randomness}

\section{Incentives}
\todo[inline]{Add block rewards and fees}
...
\todo[inline]{Merge award description}
...
%%%
	...
	Ledgers based on proof of stake can have difficulty maintaining a broad user base since most users have a vanishingly small chance of earning significant rewards. To mitigate this failing, ePoS includes a validator service award in addition to the standard staking awards to incentivize users with lower stakes to continue to maintain the network. Instead of weighting rewards by size of stake, any node which has been providing validation services for a certain number of blocks is eligible to win the award.%Is this right? There's a bit of inconsistency on how you've described this. Does everyone who has validated for a certain amount of time have the same odds of winning, or do odds increase with length of service? (Or perhaps a combination - odds increase with length of service but are capped at a certain point.) 

	One third of emitted STG tokens will be added to the service award pool, along with tokens forfeited by malicious validators and tokens from expired red packets. Every block, there is a chance that the award pool will be awarded. The odds of this occurring starts small and increases with each block. Thus, the precise time of the drawing cannot be known in advance, but on average the validator service award will be distributed every few thousand blocks.
...
\todo[inline]{Merge the description of red packets}
...
\section{Snowball}
\todo[inline]{Move Snowball below app store, etc}
\todo[inline]{Merge Snowball}
...
\section{BlockCrunch}
\todo[inline]{Rewrite BlockCrunch}
...
The TAC prevents apps from wreaking havoc on the host smartphone. It also completely abstracts the blockchain from running applications, instead providing an interface (API) to send messages and access the wallet. The TAC tightly controls access to the outside world and ensures that spending tokens requires confirmation from the user. 
...
\todo[inline]{Merge the App Store description}
...
\todo[inline]{Merge the chat description}
...
\todo[inline]{Merge the description of marketplaces}
...
Mobile staking will not immediately be available on the launch of the mainnet. However, we are making it a priority to implement mobile staking as soon as possible. As described in the introduction \ref{Introduction}, it is important to have full mobile support for as many features of the platform as possible in order to encourage adoption. Mobile staking will drastically increases the number of validator nodes, increasing the resilience and throughput of the network. Combined with ePoS, this will help prevent the stagnation and recentralization witnessed by many blockcahin projects.
...
The Stegos team is currently working on implementing the blockchain. The development progress can be tracked online at \url{https://github.com/orgs/stegos/projects/1}. Our source code is and always be 100\% open.
...
\centering
	...
	Round 1 & 14.00\% & \$0.07 & \$9,816,428 & 140,234,685 \\
	Round 2 & 4.00\% & \$0.10 & \$4,042,148 & 40,421,480 \\
	\bottomrule[2pt]
	Total & 33\% & {} & \$15,858,576 & 330,656,165
	\end{tabular}
...
Team tokens will be locked up for 18 months from mainnet launch.
