...
ePoS is egalitarian because every validator which has been running for a certain amount of time has the chance to receive a \textit{Validator service award}, regardless of the size of their stake\footnote{A nominal minimum threshold will apply to prevent users from gaming the system, but since every eligible node must provide validation services to qualify, it is impossible to spam the system, as one goal of the \textit{Validator service award} is to maximize the number of different nodes.}.
...
One third (1/3) of emitted STG tokens will be added to the service award pool, along with tokens from expired red packets. Every block, there is a chance that the award pool will be awarded. The odds of this occurring starts small and increases with each block. Thus, the precise time of the drawing cannot be known in advance, but on average the validator service award will be distributed every 5-10 days.

% Based on inflation of 14% and initial emission of 1B, we are adding around 38K tokens daily. Assuming a price of $0.05, that would be about $2k/day and $14k/week.
...
\section{BlockCrunch}
\todo[inline]{Polish the description of BlockCrunch}
...
- Bitcoin-style smart contracts for conditional and flexible spending of the payments.
- Dandelion.
- State sharding?
...
\section{Bitcoin-style smart contracts}\label{sec:smart-contracts}
