...
\author{Stegos AG}
\date{\today \\v1.1\\ \vspace{10pt}\colorlet{urllinkcolor}{white}\url{https://stegos.com/docs/whitepaper}}
...
\section{Abstract}
Cryptocurrencies and the wider crypto ecosystem promise privacy and freedom, but have yet to fully reckon with the true power of the unchecked surveillance state. Armed with nearly exhaustive data and metadata about their citizens, governments can easily thwart the pseudonymous protections offered by Bitcoin and even by privacy coins.
 
With no way to communicate within these platforms, users are forced to use insecure messaging services to contact other users to arrange their transactions, rendering the privacy protections afforded by the cryptocurrency itself useless. At the same time, exchanges act as gatekeepers to crypto markets, forcing users to give up extraordinary amounts of data and metadata which can then be used to track their activities. 

The Stegos Privacy Platform (Stegos) combines a unique blockchain and token design to ensure users can protect all of their data and metadata. Stegos is the world’s first public blockchain to offer fully anonymous messaging in addition to payment transactions, allowing its users to communicate and transact in complete privacy. 

Stegos is fully scalable and prunable, ensuring the chain always remains small without compromising trust. Spent UTXOs are removed from the chain, which is further compacted via sharding. This ensures Stegos remains completely private, secure, efficient, and environmentally sustainable.

Stegos extends this blockchain foundation to provide a platform for building privacy applications that communicate via messages sent with minimal latency. On-chain P2P trading allows privacy-conscious users to trade without being forced to leak personal data via exchanges. Last but not least, Stegos marketplaces, chat, and red packets replicate the most popular features of modern multipurpose apps in a decentralized way, allowing users to transact and communicate in complete privacy.

Stegos is secured by the gamified Proof-of-stake (gPoS) consensus mechanism based on verifiably unbiased distributed randomness. gPoS allows anyone to earn tokens for maintaining the network, even without a high stake. This encourages an extremely wide user base by ensuring that users are properly incentivized to validate the network.
...
To showcase the power of the platform, Stegos is developing its own native multi-purpose mobile application that integrates a wallet, one-on-one and group chat, the TAC, the Privacy App Store and the Stegos red packets \ref{sec:red-packets}. This app will be the gateway for users onto the Stegos Privacy Platform.
...
\section{Emission}
STG token emission will start at 14\% of the initial supply every year  during the first 4 years, and halve every 4 years thereafter. The newly created tokens will be used for block rewards and the validator service award (Section \ref{sec:service-award}).
...
\def\ExchOnePrice{0.0}
\def\ExchTwoPrice{0.0}

\def\ExchOneAlloc{0.1402}
\def\ExchTwoAlloc{0.0404}
...
\begin{tabular}{@{\extracolsep{4pt}}lllllll}
	...
	\toprule[1pt] 
	{} & \multicolumn{6}{c}{Bonus tokens for private sale rounds} \\
	\cmidrule{2-7}
	Round & Tokens & Price & \% Tokens & \% for Sale & Amount & \% Raise \\
	\midrule[1pt]
	Round 1 & \pAmt{\ExchOneTokens} & \pPrice{\ExchOnePrice} & \pPct{\ExchOneAlloc} & \pPct{\ExchOnePctSale} & \$\pAmt{\ExchOneAmt} & \pPct{\ExchOnePctRaise} \\
	Round 2 & \pAmt{\ExchTwoTokens} & \pPrice{\ExchTwoPrice} & \pPct{\ExchTwoAlloc} & \pPct{\ExchTwoPctSale} & \$\pAmt{\ExchTwoAmt} & \pPct{\ExchTwoPctRaise} \\
	\bottomrule[1pt]
