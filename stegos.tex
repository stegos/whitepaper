...
% Tokens for sale
\pgfmathparse{\InitialSupply* \PctTokensForSale / 100.0}
\edef\TokensForSale{\pgfmathresult}
...
% Total raised
\pgfmathparse{\SeedAmt + \ROneAmt + \RTwoAmt}
\edef\SoldAmt{\pgfmathresult}

% Seed allocation
\pgfmathparse{\SeedAmt / \SeedPrice / \InitialSupply}
\edef\SeedAlloc{\pgfmathresult}

% Round 1 allocation
\pgfmathparse{\ROneAmt / \ROnePrice / \InitialSupply}
\edef\ROneAlloc{\pgfmathresult}

% Round 2 allocation
\pgfmathparse{\RTwoAmt / \RTwoPrice / \InitialSupply}
\edef\RTwoAlloc{\pgfmathresult}

% Sold allocation
\pgfmathparse{\SeedAlloc + \ROneAlloc + \RTwoAlloc}
\edef\SoldAlloc{\pgfmathresult}

% Seed tokens
\def\SeedTokens{150000000}

% Round 1 tokens
\pgfmathparse{\InitialSupply * \ROneAlloc}
\edef\ROneTokens{\pgfmathresult}

% Round 2 tokens
\pgfmathparse{\InitialSupply * \RTwoAlloc}
\edef\RTwoTokens{\pgfmathresult}

% Total tokens
\pgfmathparse{\SeedTokens + \ROneTokens + \RTwoTokens}
\edef\SoldTokens{\pgfmathresult}

% Seed percentage sold
\pgfmathparse{\SeedTokens / \TokensForSale}
\edef\SeedPctSold{\pgfmathresult}

% Round 1 percentage sold
\pgfmathparse{\ROneTokens / \TokensForSale}
\edef\ROnePctSold{\pgfmathresult}

% Round 2 percentage for sale
\pgfmathparse{\RTwoTokens / \TokensForSale}
\edef\RTwoPctSold{\pgfmathresult}

% Total percentage for sale
\pgfmathparse{\SeedPctSold + \ROnePctSold + \RTwoPctSold}
\edef\TotalPctSold{\pgfmathresult}
...
% Exchange allocation 
\pgfmathparse{\ExchOneAlloc + \ExchTwoAlloc}
\edef\ExchAlloc{\pgfmathresult}

% Stage 1 tokens
\pgfmathparse{\ExchOneAlloc * \InitialSupply}
\edef\ExchOneTokens{\pgfmathresult}

% Stage 2 tokens
\pgfmathparse{\ExchTwoAlloc * \InitialSupply}
\edef\ExchTwoTokens{\pgfmathresult}

% Tokens sold 
\pgfmathparse{\ExchOneTokens + \ExchTwoTokens}
\edef\ExchTokens{\pgfmathresult}

% Stage 1 amount
\pgfmathparse{\ExchOneTokens * \ExchOnePrice}
\edef\ExchOneAmt{\pgfmathresult}

% Stage 2 amount
\pgfmathparse{\ExchTwoTokens * \ExchTwoPrice}
\edef\ExchTwoAmt{\pgfmathresult}

% Total sold
\pgfmathparse{\ExchOneAmt + \ExchTwoAmt}
\edef\ExchAmt{\pgfmathresult}

% Stage 1 percentage for sale
\pgfmathparse{\ExchOneTokens / \TokensForSale}
\edef\ExchOnePctSale{\pgfmathresult}

% Stage 2 percentage for sale
\pgfmathparse{\ExchTwoTokens / \TokensForSale}
\edef\ExchTwoPctSale{\pgfmathresult}

% Total percentage for sale
\pgfmathparse{\ExchOnePctSale + \ExchTwoPctSale}
\edef\ExchPctSale{\pgfmathresult}

%%% Global totals

% Tokens
\pgfmathparse{\SoldTokens + \ExchTokens}
\edef\TotalTokens{\pgfmathresult}

% Amount
\pgfmathparse{\SoldAmt + \ExchAmt}
\edef\TotalAmt{\pgfmathresult}

% Allocation
\pgfmathparse{\SoldAlloc + \ExchAlloc}
\edef\TotalAlloc{\pgfmathresult}

% Percentage for sale
\pgfmathparse{\TotalPctSold + \ExchPctSale}
\edef\TotalPctSale{\pgfmathresult}

% Seed percentage of raise
\pgfmathparse{\SeedAmt / \TotalAmt}
\edef\SeedPctRaise{\pgfmathresult}

% Round 1 percentage of raise
\pgfmathparse{\ROneAmt / \TotalAmt}
\edef\ROnePctRaise{\pgfmathresult}

% Round 2 percentage of raise
\pgfmathparse{\RTwoAmt / \TotalAmt}
\edef\RTwoPctRaise{\pgfmathresult}

% Total percentage of raise
\pgfmathparse{\SeedPctRaise + \ROnePctRaise + \RTwoPctRaise}
\edef\TotalPctRaise{\pgfmathresult}

% Stage 1 percentage of raise
\pgfmathparse{\ExchOneAmt / \TotalAmt}
\edef\ExchOnePctRaise{\pgfmathresult}

% Stage 2 percentage of raise
\pgfmathparse{\ExchTwoAmt / \TotalAmt}
\edef\ExchTwoPctRaise{\pgfmathresult}

% Total percentage of raise
\pgfmathparse{\ExchOnePctRaise + \ExchTwoPctRaise}
\edef\ExchPctRaise{\pgfmathresult}

% Total percentage of raise
\pgfmathparse{\TotalPctRaise + \ExchPctRaise}
\edef\PctRaise{\pgfmathresult}
...
\begin{table}[htp!]
\centering
\begin{tabular}{@{\extracolsep{4pt}}\$l^l^l^l^l^l^l}
	\toprule[1pt] 
 ...
 \cmidrule{2-7}
	...
	\toprule[2pt] 
	\midrule[0pt]
	Global Total & \pAmt{\TotalTokens} & {} & \pPct{\TotalAlloc} & \pPct{\TotalPctSale} & \$\pAmt{\TotalAmt} & \pPct{\PctRaise} \\
	\bottomrule[2pt]
